...
async def show_main_page():
    ...
    if data is not None:
        ...
        if selected_city:
            avg_temp, min_temp, max_temp, city_data, season_profile, trend, anomaly_points = heavy_func(selected_city,
                                                                                                        data)
            ...
            plot_temperature_series_with_anomalies(city_data, city_data['MA'], anomaly_points)
            ...
            if api_key:
                current_temp, temp_status = await is_normal_async(selected_city, data, api_key)
                if temp_status == 'Anomaly temp':
                    st.warning(f"Текущая температура {current_temp:.2f} °C является аномальной для сезона.")
                elif temp_status == 'Normal temp':
                    st.success(f"Текущая температура {current_temp:.2f} °C находится в пределах нормы для сезона.")
                else:
...
def plot_temperature_series_with_anomalies(city_data, moving_avg, anomaly_points):
    fig, ax = plt.subplots()
    ax.plot(city_data.index, city_data['temperature'], label="Температура", color="blue")
    ax.plot(city_data.index, moving_avg, label="Скользящее среднее", color="orange")
    if not anomaly_points.empty:
        ax.scatter(anomaly_points['timestamp'], anomaly_points['temperature'], color="red", label="Аномалии", zorder=5)
    ax.legend()
    ax.set_xlabel("Дата")
...
def plot_seasonal_profile(season_profile):
    fig, ax = plt.subplots()
    ax.errorbar(season_profile['season'], season_profile['average'], yerr=2*season_profile['std'], fmt='-o', capsize=5, label="Средняя температура")
    ax.set_xlabel("Сезон")
    ax.set_ylabel("Температура (°C)")
